...
RespStatus = typing_extensions.Literal[
    "correct", "incorrect", "partial", "instructions"
]
...
def get_partials(puzzle: m.Puzzle) -> typing.Dict[str, AnswerStr]:
    return {a.canon: a for a in (AnswerStr.from_true(r) for r in puzzle.all_answers)}
...
def submit_puzzle_answer(
    ...
    *,
    ...
    pseudoanswers: typing.Dict[str, Response] = None,
    previous_guesses: typing.Optional[typing.List[m.PuzzleSubmission]] = None,
) -> Response:
    """
    ...
    if not answer.canon:
        return Response(
            answer, "incorrect", "All responses must contain letters and/or numbers."
        )
    # magic answer used by admin team -> pretend they submitted the right answer
    if (
        maybe_team
        and maybe_team.is_admin
        and answer.canon == settings.SPOILR_ADMIN_MAGIC_ANSWER
    ):
    ...
    if response.status == "correct":
        ...
        _handle_puzzle_incorrect_answer(
            maybe_team, puzzle, answer.raw, previous_guesses=previous_guesses
        )
    else:
        _handle_puzzle_non_answer(maybe_team, puzzle, answer.raw, response.status)
    return response
...
def _handle_puzzle_non_answer(maybe_team, puzzle, answer, status):
    kind = "pseudoanswer" if status == "instructions" else "partial answer"
    dispatch(
...
def submit_minipuzzle_answer(
    team, puzzle, minipuzzle_ref, *, noop_submission, answer, correct_answer
):
    ...
    if team.is_admin and canonical_answer == settings.SPOILR_ADMIN_MAGIC_ANSWER:
        logger.info(
            f"admin team {team} used magic answer to solve minipuzzle {puzzle.external_id} {minipuzzle_ref}"
        )
    ...
    dispatch(
        ...
        team=team,
        puzzle=puzzle,
        object_id=f"{puzzle.slug}:{minipuzzle_ref}",
        message=f"Attempted minipuzzle {minipuzzle_ref} with answer "
        f"“{answer}” for {puzzle} "
        "correct"
        if is_correct
        else "incorrect",
